...
boxr=1000*lambdaD #cylinder radius
...
for i in numpy.arange(numparticles):
	names.append('g%s'%i)
dustdict = {}#{name: Dust(md,radd,lambdaD,phia,Zd*e,[lambdaD*numpy.random.randint(-6,6),lambdaD*numpy.random.randint(-6,6),0.00038257340070632547],[0,0,0],[0,0,0]) for name in names}
simtime=0.1 #simulation time in seconds
iterations=simtime/dt
timedrop=[] #Time at which the ith particle is dropped
for i in numpy.arange(numparticles):
	timedrop.append(i*0.01)
position=[]
vel=[]
acc=[]
...
newy=[j for i in newy for j in i]
newz=[j for i in newz for j in i]
t=numpy.array([0.,0.,0.]+list(t))
newx=[0.,0.,0.]+newx
newy=[0.,0.,0.]+newy
newz=[0.,0.,0.]+newz
