...
dipolepos = [0, 0,
             -0.00]  # If [0,0,0] then dipole is at original position where finite size of approx 0.003m with field strength 0.014 at edge
...
Bmom = ((2 * math.pi * (0.003) ** 3) * B / mu0) * numpy.array(
    [0, 0, 1])  # Change magnitude of magnetic moment assuming magnet just got weaker
magBmom = numpy.sqrt(Bmom[0] ** 2 + Bmom[1] ** 2 + Bmom[2] ** 2)
Bmomhat = numpy.array(Bmom) / magBmom

# Constants following from Joe's thesis
LAMBDA = numpy.sqrt(magBmom * mu0 * e / (
4 * math.pi * me * numpy.sqrt(kb * Te * 3 / me)))  # Distance at which electron is influenced from Joe's thesis
...
def voidvol(rmax):
    intfunc = lambda r: 2 * math.pi * r * numpy.sqrt(
        (e * curlyM * r ** 2 / (2 * numpy.sqrt(e * curlyM * VeT * me) + me * r * VeT)) ** (2. / 3.) - r ** 2)
    voidvol = scipy.integrate.quad(intfunc, 0, rmax)
    return voidvol[0]


def voidQ(rmax):
    Qfunc = lambda r: e * ne0 * numpy.exp((e / (kb * Te)) * electrodeV * ((1. - (1. / sheathd) * numpy.sqrt(
        (e * curlyM * r ** 2 / (2 * numpy.sqrt(e * curlyM * VeT * me) + me * VeT * r)) ** (2. / 3.) - r ** 2)) ** 2 + (
                                                                          r / boxr) ** 2)) * 2 * math.pi * r * numpy.sqrt(
        (e * curlyM * r ** 2 / (2 * numpy.sqrt(e * curlyM * VeT * me) + me * VeT * r)) ** (2. / 3.) - r ** 2)
...
chargepos = pospos()
numbcharge = len(chargepos[
                     0]) * 2 * math.pi * boxr / lambdaD  # The factor multiplied by is the number of 2D slices in the r-z plane there are in the 3D volume
...
def checkedenofm():  # Run this function to calculate ratio of charge in the void that is displaced compared to outside void in rest of sheath
    print("Ratio of charge inside to outside region of inaccessibility=",
          voidcharge / (ne0 * e * (2 * math.pi * boxr * sheathd - voidvol(rmax))))
    print("Approximate volume using half doughnut calculation is", 2 * math.pi * (rmax / 2) * math.pi * rmax ** 2 / 2,
          "should be bigger than integral of my solid, which is", voidvol(rmax))
