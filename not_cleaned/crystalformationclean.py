...
class Dust:
    def __init__(self, mass=0, radius=0, lambdaD=0, phia=phia, initcharge=0, initpos=[0, 0, 0], initvel=[0, 0, 0],
                 initacc=[0, 0, 0]):
        self.m = mass
        self.rad = radius
    ...
    def scatterwalls(self):
        if abs(numpy.array(self.pos[0]) + numpy.array(self.vel[0]) * dt + 0.5 * numpy.array(
                self.acc[0]) * dt ** 2) + abs(
                            numpy.array(self.pos[1]) + numpy.array(self.vel[1]) * dt + 0.5 * numpy.array(
                self.acc[1]) * dt ** 2) > boxr:
    ...
    def step(self):  # Equation of motion involving forces, to update position and velocitiy and acceleration in time
        ...
        self.acc = numpy.array(self.acc) + accsheath + accradial + (
            E * numpy.array(unitr) * self.charge / self.m) + self.verticalion()
    ...
    def steptest(self, Emultifield=numpy.array(
        [0, 0, 0])):  # Test function for interactions, alter incooporate into step function
    ...
    def verticalion(self):
        if self.pos[2] < sheathd:
            ...
            return numpy.array([0, 0, (
                ((Ti / e) ** 2) * numpy.log(
                    lambdade * mach ** 2 / (beta * lambdadi)) * beta ** 2 / mach ** 2) / self.m])
        else:
            return numpy.array([0, 0, 0])
    ...
    def intergraind(self, g2):
        if numpy.sqrt((self.pos[0] - g2.pos[0]) ** 2 + (self.pos[1] - g2.pos[1]) ** 2 + (
                    self.pos[2] - g2.pos[2]) ** 2) <= radinfluence:
            return True
        else:
            return False

    def EXBacc(self, B=[0, 0, 0.014],
               machmag=0.):  ##Ion drag force due to Constant vertical B field causing ExB drift of ions
        if self.pos[2] < sheathd and self.Bswitch == True:
            ...
            if machmag <= 2:
                ...
                forceS = (1. / 3.) * numpy.sqrt(32 * math.pi) * (
                    (Ti * kb / e) ** 2) * coloumblog * beta ** 2 * e0 * mach
                phi = abs(self.charge * e / (kb * Te * 4 * math.pi * e0 * self.rad))
                forceC = mach * vT * 4 * math.pi * self.rad ** 2 * ni0 * mi * numpy.sqrt(
                    kb * Te / (2 * math.pi * me)) * numpy.exp(-phi)
                forceN = -numpy.array(self.vel) * mi * numpy.sqrt(4 * math.pi) * ni0 * numpy.sqrt(
                    kb * Ti / (2 * math.pi * mi)) * math.pi * radd ** 2
                nn = 7. / (kb * Ti)  # Number density neutrals, 7Pa used by Saitou
                forceionneutraldrag = mach * mi * numpy.sqrt(4 * math.pi) * nn * numpy.sqrt(
                    kb * Ti / (2 * math.pi * mi)) * math.pi * radd ** 2
    ...
    def EXBaccfortov(self, B=[0, 0, 0.014], machmag=0.):  ##Test other literature theories
        if self.pos[2] < sheathd and self.Bswitch == True:
            ...
            if machmag <= 1 and beta <= 13:
                ...
                force = abs((beta ** 2 * numpy.log(lambdade * machmag ** 2 / (lambdaD * beta)) * (
    ...
    def EXBacckinetic(self, B=[0, 0, 0.014], machmag=0.):
        if self.pos[2] < sheathd and self.Bswitch == True:
            ...
            beta = abs(Zd * e ** 2 / (LAMBDA * Ti * kb))
            E = self.radialfield()
            incollfreq = e * numpy.sqrt(E[0] ** 2 + E[1] ** 2 + E[2] ** 2) / (
                mi * numpy.sqrt(vdrift[0] ** 2 + vdrift[1] ** 2 + vdrift[2] ** 2))
            l = vT / incollfreq
    ...
    def EXBaccDUSTT(self, B=[0., 0., 0.014], machmag=0.):
        if self.pos[2] < sheathd and self.Bswitch == True:
            ...
            tau = Te / Ti
            coloumblog = 5.
            force = 2 * math.pi * radd ** 2 * mi * ni0 * vT * numpy.sqrt(2) * (
                vdrift - numpy.array(self.vel)) * numpy.log(coloumblog) * (-phia) * chandra(machmag / numpy.sqrt(2)) / (
                        Ti * (machmag / numpy.sqrt(2)) / Te)
            return force / self.m
        else:
            return numpy.array([0, 0, 0])  # raise ValueError("Dust particle is not in sheath")
    ...
    def combinedrift(self, B=[0, 0, 0.014]):
        if self.Bswitch == True:
            ...
            gradB = (
                        -3. * mu0 * numpy.sqrt(9 * self.pos[2] ** 2 + magBmom ** 2) / (
                        4 * math.pi * r ** 4)) * numpy.array(
                [numpy.cos(theta), numpy.sin(theta), 0.]) \
                    + (mu0 * 18 * self.pos[2] / (
                8 * math.pi * r ** 3 * numpy.sqrt(9 * self.pos[2] ** 2 + magBmom ** 2))) * numpy.array([0, 0, 1])
...
def generating_list_of_particles():
    ...
    for i in numpy.arange(len(particlesperringfinal)):
        ...
        for j in numpy.arange(particlesperringfinal[i]):
            initpositions.append(
                [ringsizefinal[i] * numpy.cos(j * dtheta), ringsizefinal[i] * numpy.sin(j * dtheta), boxz])
...
def drop_particles_into_cylinder():
...
def save_the_positions_in_pickle():
...
iterations = 3000
inititerations = 3000  # Number of iterations for particles to settle down

print("Generating list of particles...")
dustdict = generating_list_of_particles()
print("Drop particles into cylinder...")
pairs = drop_particles_into_cylinder()
