...
Bmomstrength = numpy.arange(0.005,0.025,0.001)
numpy.delete(Bmomstrength,9)
def norm(x):
...
for i in numpy.arange(len(Bmomstrength)):
    ...
    gibtemp.const.Bmom = ((2 * math.pi * (0.003) ** 3) * Bmomstrength[i] / gibtemp.const.mu0) * numpy.array(
    [0, 0, 1])
    ...
    gibtemp.savetopickle(Bmomstrength[i],True)
