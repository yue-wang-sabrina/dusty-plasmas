...
class SpectrumCanvas(FigureCanvas):
    ...
    def display(self): #Creates interactive animation (including EMTS and sensor fusion reconstruction)
            ...
            if os.path.exists("simulatedsensor.obj"):            
                ...
                self.maximum=max([max((numpy.array(fuse.globalposition)[:,0])),max((numpy.array(fuse.globalposition)[:,1])),max((numpy.array(fuse.globalposition)[:,2]))])    
                self.minimum=min([min(numpy.array(fuse.globalposition)[:,0]),min(numpy.array(fuse.globalposition)[:,1]),min(numpy.array(fuse.globalposition)[:,2])])
                ...
                test.quiver3D(fuse.globalposition[0][0],fuse.globalposition[0][1],fuse.globalposition[0][2],fuse.orientationvectorx[0][0],fuse.orientationvectorx[0][1],fuse.orientationvectorx[0][2],length=abs(self.maximum*0.5),arrow_length_ratio=0.5,color='r')
                test.quiver3D(fuse.globalposition[0][0],fuse.globalposition[0][1],fuse.globalposition[0][2],fuse.orientationvectory[0][0],fuse.orientationvectory[0][1],fuse.orientationvectory[0][2],length=abs(0.5*self.maximum),arrow_length_ratio=0.5,color='k')
                test.quiver3D(fuse.globalposition[0][0],fuse.globalposition[0][1],fuse.globalposition[0][2],fuse.orientationvectorz[0][0],fuse.orientationvectorz[0][1],fuse.orientationvectorz[0][2],length=abs(0.5*self.maximum),arrow_length_ratio=0.5,color='b')
                ...
                def update_graph(p):
                    test.clear()
                    test.plot([item[0] for item in fuse.globalposition],[item[1] for item in fuse.globalposition],[item[2] for item in fuse.globalposition],color='r',label='Using orientation updates')
                    #test.plot(em.EMTSx,em.EMTSy,em.EMTSz,color='b',label='EMTS measurements')
                    test.quiver3D(fuse.globalposition[p][0],fuse.globalposition[p][1],fuse.globalposition[p][2],fuse.orientationvectorx[p][0],fuse.orientationvectorx[p][1],fuse.orientationvectorx[p][2],length=abs((self.maximum-self.minimum)*0.5),arrow_length_ratio=0.5,color='r')
                    test.quiver3D(fuse.globalposition[p][0],fuse.globalposition[p][1],fuse.globalposition[p][2],fuse.orientationvectory[p][0],fuse.orientationvectory[p][1],fuse.orientationvectory[p][2],length=abs((self.maximum-self.minimum)*0.5),arrow_length_ratio=0.5,color='k')
                    test.quiver3D(fuse.globalposition[p][0],fuse.globalposition[p][1],fuse.globalposition[p][2],fuse.orientationvectorz[p][0],fuse.orientationvectorz[p][1],fuse.orientationvectorz[p][2],length=abs((self.maximum-self.minimum)*0.5),arrow_length_ratio=0.5,color='b')
                    ...
                    test.set_title("Reconstruction from simulated sensor data")
                    test.legend()
                self.ani = matplotlib.animation.FuncAnimation(self.fig, update_graph,frames=self.anilen,interval=10,blit=False)
                self.draw()
...
class SFGui(Ui_Dialog): #Setting up/Connecting the gui buttons and connecting them to their associated functions
    ...
    def combobox(self): #This refers to the dropdown list with all the simple motions. Here they are pre-made so you don't have to retype them everytime.
        ...
        if self.comboBox.currentText() == "Rotate x 90deg":
            timenew=0.5
            self.count+=1
            self.listWidget.addItem("Rotational acceleration added"+str(rotatex)+"x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
                self.allacc.append([rotatex,1,self.count])
            
            self.count+=1
            self.listWidget.addItem("Rotational acceleration added"+str(rotatexm)+"x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
                self.allacc.append([rotatexm,1,self.count])
            
            self.count+=1
            self.listWidget.addItem("Rotational acceleration added"+str(nochange)+"x %i"%int(1)+", %i"%self.count)
            ...
            timenew=0.5
            self.count+=1
            self.listWidget.addItem("Rotational acceleration added"+str(rotatey)+"x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
                self.allacc.append([rotatey,1,self.count])
            
            self.count+=1
            self.listWidget.addItem("Rotational acceleration added"+str(rotateym)+"x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
            ...
            timenew=0.5
            self.count+=1
            self.listWidget.addItem("Rotational acceleration added"+str(rotatez)+"x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
                self.allacc.append([rotatez,1,self.count])
            
            self.count+=1
            self.listWidget.addItem("Rotational acceleration added"+str(rotatezm)+"x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
            ...
            timenew=1.
            self.count+=1
            self.listWidget.addItem("Linear acceleration added:"+ str(accx)+ "x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
                self.allacc.append([accx,0,self.count])
            
            self.count+=1
            self.listWidget.addItem("Linear acceleration added:"+ str(accxm)+ "x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
            ...
            timenew=1.
            self.count+=1
            self.listWidget.addItem("Linear acceleration added:"+ str(accy)+ "x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
                self.allacc.append([accy,0,self.count])
            
            self.count+=1
            self.listWidget.addItem("Linear acceleration added:"+ str(accym)+ "x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
            ...
            timenew=1.
            self.count+=1
            self.listWidget.addItem("Linear acceleration added:"+ str(accz)+ "x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
                self.allacc.append([accz,0,self.count])
            
            self.count+=1
            self.listWidget.addItem("Linear acceleration added:"+ str(acczm)+ "x %i"%int(timenew/self.dt)+", %i"%self.count)
            for k in numpy.arange(int(timenew/self.dt)):
    ...
    def linearacc(self): #Button for User input linear jerk and time period in seconds
        ...
        if len(text2check)==3:
            if isinstance(eval(text2check[0]),float) or isinstance(eval(text2check[0]),int) and isinstance(eval(text2check[1]),float) or isinstance(eval(text2check[1]),int) and isinstance(eval(text2check[2]),float) or isinstance(eval(text2check[2]),int): #If all 3 inputs are either floats or integers
                ...
                if len(time2.split())==1:
                    if isinstance(eval(time2.split()[0]),float) or isinstance(eval(time2.split()[0]),int): #If the input is either a float or integer
                        ...
                        self.listWidget.addItem("Linear acceleration added:"+ str(text2)+ "x %i"%int(timenew/self.dt)+", %i"%self.count) #Display this motion to listwidget
                        for k in numpy.arange(int(timenew/self.dt)): #timenew/self.dt = no. iterations the jerk lasts for
                            self.allacc.append([text2,0,self.count]) #Add jerk to list containing all user inputs. The 0 assigns the acceleration as linear. 
    ...
    def rotacc(self): #Button for user input rotational acceleration and time period in seconds. Same method as that for linear jerk
        text,ok = QInputDialog.getText(dialog, 'User Input','Enter rotational acceleration (e.g. 1 0 0)')
        textcheck=text.split()
        if len(textcheck)==3:
            if isinstance(eval(textcheck[0]),float) or isinstance(eval(textcheck[0]),int) and isinstance(eval(textcheck[1]),float) or isinstance(eval(textcheck[1]),int) and isinstance(eval(textcheck[2]),float) or isinstance(eval(textcheck[2]),int):
                ...
                if len(time.split())==1:
                    if isinstance(eval(time.split()[0]),float) or isinstance(eval(time.split()[0]),int):
                        ...
                        self.count+=1
                        self.listWidget.addItem("Rotational acceleration added"+str(text)+"x %i"%int(timenew/self.dt)+", %i"%self.count)
                        for k in numpy.arange(int(timenew/self.dt)):
    ...
    def clearall(self): #Attempt at resetting, but still problematic... Can tell when it does not work : it just crashes
        self.count=0
        self.allacc=[]
        self.graph.clear()
